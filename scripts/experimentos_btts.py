...
def evaluar(nombre_cfg, cols_extra):
    ...
    for nombre, cfg in EM.MERCADOS.items():
        ...
        marca = "  <== ambos_marcan" if nombre == "ambos_marcan" else ""
        print(f"  {nombre:20s} n={r['partidos']:4d}  {r['sigmas']:+.2f}s  acierto {am*100:5.1f}% vs {amk*100:5.1f}% ({ad*100:+.1f}pp){marca}")
    print(f"  SUMA sigmas: {suma_sigmas:+.2f}   SUMA acierto_dif: {suma_acierto*100:+.1f}pp\n")

evaluar("base+elo", [])
evaluar("base+elo+btts (sola)", grupos['btts'])
evaluar("produccion", prod)
evaluar("produccion+btts", prod + grupos['btts'])
