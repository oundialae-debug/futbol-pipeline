...
def obtener_partidos_recientes(team_id: int, intentos: int = 3) -> list[dict]:
    for intento in range(intentos):
        ...
        if r.status_code == 429:
            espera = 5 * (intento + 1)
            print(f"  [aviso] límite de peticiones (429) para equipo {team_id} -> esperando {espera}s (intento {intento+1}/{intentos})")
            time.sleep(espera)
            continue
        print(f"  [aviso] fallo al pedir equipo {team_id}: {r.status_code}")
        return []
    print(f"  [aviso] equipo {team_id} sigue dando 429 tras {intentos} intentos -> se omite esta vez")
    return []
...
def main():
    historico = pd.read_csv(RUTA_CSV) if os.path.exists(RUTA_CSV) and os.path.getsize(RUTA_CSV) > 0 else pd.DataFrame()
